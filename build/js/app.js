...
/* harmony import */ var _components_suscribeBox__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(/*! ./components/suscribeBox */ "./assets/js/components/suscribeBox.js");
/* harmony import */ var _components_tienda__WEBPACK_IMPORTED_MODULE_8__ = __webpack_require__(/*! ./components/tienda */ "./assets/js/components/tienda.js");
...
jQuery(function ($) {
  $(document).ready(function () {
    ...
    Object(_components_suscribeBox__WEBPACK_IMPORTED_MODULE_7__["default"])($); // Open more field

    Object(_components_tienda__WEBPACK_IMPORTED_MODULE_8__["default"])($);
  });
});
  ...
  function load_posts() {
    ...
    $.ajax({
      ...
      success: function success(data) {
        var $data = $(data);
        pppCount += 1;
        var pppTotal = Math.ceil(parseInt(ajax_posts.total_posts.publish - ppp) / ppp);

        if (pppTotal == pppCount) {
        ...
        if ($data) {
          ...
          $("#more_posts").removeClass('btn-loader');
        } else {
          $("#more_posts").hide();
          $("#more_posts").attr("disabled", true);
        }
...
/***/ "./assets/js/components/suscribeBox.js":
/*!*********************************************!*\
  !*** ./assets/js/components/suscribeBox.js ***!
  \*********************************************/
/*! exports provided: default */
...
"use strict";
__webpack_require__.r(__webpack_exports__);
var suscribeBox = function suscribeBox($) {
  $('#mailpoet_form_2 .mailpoet_paragraph input[type="email"]').click(function () {
    $('#mailpoet_form_2 .mailpoet_form .mailpoet_form_columns').css('flex-direction', 'column');
    $('#mailpoet_form_2 .mailpoet_form .mailpoet_form_columns .mailpoet_form_column div.mailpoet_paragraph').slideDown();
    $('#mailpoet_form_2 .mailpoet_form_columns .mailpoet_form_column .mailpoet_paragraph').addClass('campos-activos');
  });
};

/* harmony default export */ __webpack_exports__["default"] = (suscribeBox);

/***/ }),

/***/ "./assets/js/components/tienda.js":
/*!****************************************!*\
  !*** ./assets/js/components/tienda.js ***!
  \****************************************/
/*! exports provided: default */
...
var tienda = function tienda($) {
  // Cuando se hace clic en el botón de incremento
  $(document).on('click', '.button-qty[data-quantity="plus"]', function () {
    var $quantityInput = $(this).closest('.quantity').find('.qty');
    var currentQuantity = parseInt($quantityInput.val());
    var newQuantity = currentQuantity + 1; // Actualiza la cantidad en el campo de entrada

    $quantityInput.val(newQuantity);
    $quantityInput.trigger('change');
  }); // Cuando se hace clic en el botón de decremento

  $(document).on('click', '.button-qty[data-quantity="minus"]', function () {
    var $quantityInput = $(this).closest('.quantity').find('.qty');
    var currentQuantity = parseInt($quantityInput.val());
  ...
  /*   $('.single-product .quantity input[type="number"]').on('change', function() {
        var product_id = $(this).closest('.quantity').find('input[type="number"]').attr('id');
       var quantity = $(this).val();
          // Obtener la URL de admin-ajax.php desde la variable pasada por PHP 
          // Realizar la solicitud AJAX para actualizar el precio
        $.ajax({
            type: 'POST',
            dataType: "html", 
            url: ajax_posts.ajaxurl,
